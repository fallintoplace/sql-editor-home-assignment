...
test('Result expiry preserves SQL but removes row access', async () => { const f = fixture({ retentionMs: 1 }), r = f.runs.submit(owner, f.request()); await f.runs.wait(owner, r.id); await sleep(5); assert.throws(() => f.runs.result(owner, r.id), { code: 'RESULT_EXPIRED' }); assert.ok(f.runs.get(owner, r.id).sql); });
