...
const savedState = (page: Page) => page.evaluate(() => {
    ...
    return JSON.parse(localStorage.getItem('clickstudio:local-owner:demo:v1')!);
});
...
test('Exported local backups round-trip through the file picker as independent drafts', async ({ page }) => {
    ...
    expect(backup.format).toBe('clickstudio-local-drafts'); expect(backup.sourceConnection.id).toBe('demo');
    const dialog = await open(page);
...
test('Tab capacity requires an explicit smaller selection and never closes an existing draft', async ({ page }) => {
    await page.addInitScript(() => {
        ...
        localStorage.setItem('clickstudio:local-owner:demo:v1', JSON.stringify({ version: 1, tabs: drafts, activeId: drafts[0]!.id }));
    });
    const effects = sideEffects(page);
    await page.goto('/');
    const dialog = await open(page);
...
test('Invalid UTF-8 and non-local exports report actionable errors', async ({ page }) => {
    ...
    await localInput(page).setInputFiles(backupFile({ format: 'clickstudio-workspace', version: 1, documents: [] }));
...
for (const theme of ['light', 'dark']) test(`Local-file preview remains contained at 390px in ${theme} mode`, async ({ page }, testInfo) => {
    ...
    await page.screenshot({ path: testInfo.outputPath(`clickstudio-local-files-${theme}.png`), fullPage: true });
});
