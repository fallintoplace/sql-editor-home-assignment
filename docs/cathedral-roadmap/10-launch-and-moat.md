...
1. **Assignment release:** SQL, results, charts, scripts, file import.
...
ClickHouse execution truth
  ...
  = a ClickHouse workbench that teaches, debugs, and proves
```
...
Use the first four stages to finish the assignment. Use stages five through ten to turn the submission into the cathedral. Do not expand scope until the previous acceptance gate is green. Launch only when the single memorable workflow works with a clean local setup and a reproducible artifact.
