...
Never use the starter `default` identity for a shared deployment. The local Compose setup is intentionally permissive for a demo; a hosted product needs a restricted identity and server-side secret storage.
