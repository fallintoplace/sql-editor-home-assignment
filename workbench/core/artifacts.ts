...
export class ArtifactService {
    ...
    export(p: Principal) { return { format: 'cathedral-workspace', version: 1, exportedAt: new Date().toISOString(), documents: this.list(p) }; }
    import(p: Principal, value: unknown): QueryDocument[] {
        canWrite(p);
        const v = record(value);
        requireThat(v.format === 'cathedral-workspace' && v.version === 1 && Array.isArray(v.documents) && v.documents.length <= 100, 400, 'BUNDLE_FORMAT', 'Unsupported workspace bundle');
