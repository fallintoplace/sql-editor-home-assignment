...
const client = createClient({ url: env.CLICKHOUSE_URL ?? 'http://127.0.0.1:8123', username: env.CLICKHOUSE_ADMIN_USER ?? 'workbench_admin', password: env.CLICKHOUSE_ADMIN_PASSWORD, request_timeout: 30000 });
async function command(query: string) { await client.command({ query, query_id: `cathedral-setup-${randomUUID()}`, abort_signal: AbortSignal.timeout(30000) }); }
try {
    ...
    for (const [user, password, profile] of [
        ['workbench_reader', env.CLICKHOUSE_PASSWORD!, ' SETTINGS PROFILE cathedral_reader'],
        ['workbench_writer', env.CLICKHOUSE_WRITER_PASSWORD!, ''],
    ]) {
    ...
    await command('GRANT SELECT ON default.events TO workbench_reader');
    await command('GRANT SELECT ON default.import_events TO workbench_reader');
    for (const table of ['columns', 'tables', 'databases', 'processes'])
        await command(`GRANT SELECT ON system.${table} TO workbench_reader`);
    await command('GRANT INSERT ON default.import_events TO workbench_writer');
