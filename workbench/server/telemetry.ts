...
export async function startTelemetry(url?: string) { if (!url)
    return async () => { }; const sdk = new NodeSDK({ resource: resourceFromAttributes({ 'service.name': 'cathedral-workbench' }), traceExporter: new OTLPTraceExporter({ url }) }); sdk.start(); return () => sdk.shutdown(); }
...
export const telemetry: RequestHandler = (req, res, next) => {
    const span = trace.getTracer('cathedral-workbench').startSpan('workspace.request');
