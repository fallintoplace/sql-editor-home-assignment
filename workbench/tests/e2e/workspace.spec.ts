...
test('Pending edits survive page suspension and a reload', async ({ page }) => {
    ...
    const saved = await page.evaluate(() => JSON.parse(localStorage.getItem('cathedral:local-owner:demo:v1')!));
...
test('Malformed saved metadata preserves valid SQL and leaves the editor usable', async ({ page }) => {
    await page.addInitScript(() => {
        localStorage.setItem('cathedral:local-owner:demo:v1', JSON.stringify({
