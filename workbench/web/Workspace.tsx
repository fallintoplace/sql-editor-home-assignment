...
import type { Connection, ProfilePipeline, QueryDocument, Run, RunEvent, Schema, Script, Published } from '../shared/types';
...
const learnerExamples: Array<{ name: string; level: ExperienceLevel; description: string; sql: string; chart: Draft['chart']; parameters?: Record<string, string> }> = [
    ...
    { name: '03 · Filter and sort.sql', level: 'beginner', description: 'Keep only matching rows, then order the result for easy reading.', sql: 'SELECT\n    number AS event_id,\n    number * 10 AS value\nFROM numbers(12)\nWHERE number % 2 = 0\nORDER BY value DESC', chart: { kind: 'table', x: 0, ys: [], title: 'Filtered events' } },
    ...
    { name: '05 · Join two small datasets.sql', level: 'beginner', description: 'Join two generated datasets to see how matching keys combine columns.', sql: 'WITH\n    event_visits AS (SELECT number AS id, number * 10 AS visits FROM numbers(5)),\n    user_scores AS (SELECT number AS id, number + 100 AS score FROM numbers(5))\nSELECT\n    event_visits.id,\n    event_visits.visits,\n    user_scores.score\nFROM event_visits\nINNER JOIN user_scores USING (id)\nORDER BY id', chart: { kind: 'table', x: 0, ys: [], title: 'Joined datasets' } },
    { name: '06 · Rolling signal lab.sql', level: 'beginner', description: 'Go bigger: generate events, label them, calculate a rolling average, and format a compact signal.', sql: "WITH base AS (\n    SELECT\n        toDateTime('2026-01-01 00:00:00') + number * 60 AS minute,\n        ['search', 'checkout', 'signup'][number % 3 + 1] AS event,\n        (number * 37) % 100 + 10 AS requests\n    FROM numbers(30)\n), windowed AS (\n    SELECT\n        minute,\n        event,\n        requests,\n        round(avg(requests) OVER (\n            PARTITION BY event\n            ORDER BY minute\n            ROWS BETWEEN 2 PRECEDING AND CURRENT ROW\n        ), 1) AS rolling_requests\n    FROM base\n)\nSELECT\n    minute,\n    event,\n    requests,\n    rolling_requests,\n    arrayStringConcat(\n        arrayMap(x -> toString(x), [requests, rolling_requests]),\n        ' → '\n    ) AS signal\nFROM windowed\nORDER BY minute", chart: { kind: 'line', x: 0, ys: [2, 3], title: 'Rolling request signal' } },
    { name: '07 · Query parameter.sql', level: 'expert', description: 'Use a typed parameter so the same query can answer different windows.', sql: 'WITH {days:UInt32} AS days\nSELECT\n    number AS day,\n    number * 4 AS events\nFROM numbers(days)\nORDER BY day', chart: { kind: 'line', x: 0, ys: [1], title: 'Parameterized events' }, parameters: { days: '7' } },
    { name: '08 · Explain a query.sql', level: 'expert', description: 'Inspect the plan before running a bounded read-only query.', sql: "EXPLAIN indexes = 1\nSELECT\n    toDate('2026-01-01') + number AS day\nFROM numbers(14)\nWHERE number % 2 = 0\nORDER BY day", chart: { kind: 'table', x: 0, ys: [], title: 'Query plan' } },
];
const beginnerDemoPipeline: ProfilePipeline = {
    available: true,
    source: 'query_shape',
    nodes: [
        { id: 'demo-read', label: 'Read numbers(7)', kind: 'read', status: 'estimated', rows: '7' },
        { id: 'demo-project', label: 'Create day and events', kind: 'stage', status: 'estimated', detail: 'toDate + number and (number + 1) × 10' },
        { id: 'demo-sort', label: 'Sort by day', kind: 'sort', status: 'estimated' },
        { id: 'demo-output', label: 'Return 7 rows', kind: 'output', status: 'estimated', rows: '7' },
    ],
    edges: [
        { source: 'demo-read', target: 'demo-project' },
        { source: 'demo-project', target: 'demo-sort' },
        { source: 'demo-sort', target: 'demo-output' },
    ],
    notice: 'Demo-only query shape. No SQL has run and these stages are not live ClickHouse telemetry.',
};
export function Workspace({ connection, dark, experience, refresh, copy }: {
    ...
    const closeTab = (draftId: string) => { setState(s => closeDraft(s, draftId)); setNotice('Tab closed locally. Reopen closed tab restores it. The last 10 closed tabs are retained; server revisions and query history are unchanged.'); };
    ...
    const showPanel = (next: Panel) => { setFocusMode(false); setWidgetMenu(false); setExamplesOpen(false); setResultDeck('closed'); setPanel(next); };
    ...
    const openResults = (view: Exclude<ResultDeckView, 'closed'>) => { setPanel(null); setWidgetMenu(false); setResultDeck(view); window.setTimeout(() => jumpTo(view === 'history' ? 'query-history' : 'query-results-deck'), 0); };
    ...
    const editorError = run.data?.error && (run.data.sql === active.sql || (run.data.sourceFrom !== undefined && run.data.sourceTo !== undefined && active.sql.slice(run.data.sourceFrom, run.data.sourceTo).trim() === run.data.sql.trim())) ? run.data.error : undefined;
 ...
 <WorkspaceShell experience={experience} appName={copy.app.name} documentName={active.name} connectionName={connection.name} leftNav={workspaceLeftNav} rightNav={workspaceRightNav} footer={workspaceFooter}/>
 ...
 {examplesOpen && <section className="example-launcher" role="dialog" aria-label={copy.workspace.examples}><header className="example-launcher-header"><div><span className="eyebrow">{copy.workspace.examples}</span><h2>{copy.workspace.examplesDescription}</h2></div><Action type="empty" aria-label={copy.workspace.closeExamples} onClick={() => setExamplesOpen(false)}>×</Action></header>{experience === 'beginner' && <section className="beginner-demo-preview" aria-label="Beginner demo preview"><div className="beginner-demo-copy"><span className="eyebrow">Zero-setup demo</span><h3>See a query and its shape before you run it</h3><p>Start with seven generated days. This preview is local only: it never contacts ClickHouse and the graph is an inferred teaching aid.</p><pre>{learnerExamples[0]!.sql}</pre><div className="beginner-demo-actions"><Action type="primary" onClick={() => openLearnerExample(learnerExamples[0]!)}>Open demo query</Action><Action type={beginnerGraphOpen ? 'secondary' : 'empty'} aria-expanded={beginnerGraphOpen} onClick={() => setBeginnerGraphOpen(value => !value)}>{beginnerGraphOpen ? 'Hide mock graph' : 'Show mock graph'}</Action></div></div>{beginnerGraphOpen && <div className="beginner-demo-graph"><PipelineGraph pipeline={beginnerDemoPipeline}/></div>}</section>}<div className="example-grid">{visibleExamples.map(example => <article className="example-card" key={example.name}><div className="example-card-heading"><span className="example-level">{example.level === 'beginner' ? copy.app.beginner : copy.app.expert}</span><strong>{example.name}</strong></div><p>{example.description}</p><pre>{example.sql}</pre><Action type="primary" onClick={() => openLearnerExample(example)}>{copy.workspace.openExample}</Action></article>)}</div></section>}
 ...
 <div className={['workspace-grid', (!panel || focusMode) && 'drawer-closed', (!filesVisible || focusMode) && 'sidebar-closed', filesCompact && filesVisible && !focusMode && 'sidebar-compact'].filter(Boolean).join(' ')}><aside id="workspace-files" className={`sidebar stack ${filesCompact ? 'sidebar-compact' : ''}`} aria-label={copy.workspace.title} hidden={!filesVisible || focusMode}><h2 title={copy.workspace.title}>{copy.workspace.title}</h2><Action data-rail-icon="+" aria-label={copy.workspace.newTab} title={copy.workspace.newTab} onClick={() => addDraft(newDraft())}>{copy.workspace.newTab}</Action><Action data-rail-icon="↥" aria-label={copy.workspace.importBackup} title={copy.workspace.importBackup} onClick={openLocalFiles}>{copy.workspace.importBackup}</Action><div className="sidebar-search"><TextField aria-label={copy.workspace.searchPlaceholder} placeholder={copy.workspace.searchPlaceholder} value={search} onChange={setSearch}/></div><details className="nav-section" open aria-label={copy.workspace.savedFiles}><summary title={copy.workspace.savedFiles} data-rail-icon="▤">{copy.workspace.savedFiles}</summary>{matchingDocuments.map(d => <Action type="empty" align="left" key={d.id} onClick={() => openDocument(d)}>{d.name} <small>r{d.revision}{d.verifiedRevision ? ' · self-reviewed' : ''}</small></Action>)}
 ...
 <details className="nav-section" aria-label={copy.workspace.schema}><summary title={copy.workspace.schema} data-rail-icon="◇">{copy.workspace.schema}</summary>{schema.isFetching && <p role="status">Loading schema…</p>}{schema.error && <Callout danger>{message(schema.error)}</Callout>}<Action disabled={!connection.trusted} onClick={() => void schema.refetch()}>{copy.workspace.refreshSchema}</Action>
 ...
 {schema.data?.warnings.map((warning, i) => <p className="muted" key={i}>{warning}</p>)}</details><details className="nav-section" open aria-label={copy.workspace.statementOutline}><summary title={copy.workspace.statementOutline} data-rail-icon="≡">{copy.workspace.statementOutline}</summary>{parsed.statements.map((s, i) => <Action key={s.from} type="empty" onClick={() => { patch({ from: s.from, to: s.to }); editor.current?.focus(); }}>Statement {i + 1} · {s.sql.slice(0, 32)}</Action>)}
 </details><Action data-rail-icon="⇩" aria-label={copy.workspace.exportDrafts} onClick={exportLocalDrafts} title="Backup contains SQL and parameter values.">{copy.workspace.exportDrafts}</Action></aside>
 <main className="editor-column workspace-main"><div className="tabs" role="tablist" aria-label="SQL documents" onKeyDown={e => { if (!['ArrowLeft', 'ArrowRight', 'Home', 'End'].includes(e.key))
        return; e.preventDefault(); const index = state.tabs.findIndex(d => d.id === active.id), next = e.key === 'Home' ? 0 : e.key === 'End' ? state.tabs.length - 1 : (index + (e.key === 'ArrowRight' ? 1 : -1) + state.tabs.length) % state.tabs.length; setState(s => ({ ...s, activeId: s.tabs[next]!.id })); const tab = e.currentTarget.querySelectorAll<HTMLElement>('[role=tab]')[next]; tab?.focus(); tab?.scrollIntoView({ block: 'nearest', inline: 'nearest' }); }}>{state.tabs.map(d => <div key={d.id} className="sql-tab" role="tab" id={`sql-tab-${d.id}`} aria-controls={`sql-panel-${d.id}`} title={d.name} aria-selected={d.id === active.id} tabIndex={d.id === active.id ? 0 : -1} onClick={() => setState(s => ({ ...s, activeId: d.id }))} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setState(s => ({ ...s, activeId: d.id })); } }}><span className="sql-tab-label">{d.name}</span><button type="button" className="sql-tab-close" aria-label={`Close ${d.name}`} title={`Close ${d.name}`} disabled={busy} onClick={event => { event.stopPropagation(); closeTab(d.id); }}>×</button></div>)}</div>
 <div className="toolbar wrap editor-tab-actions" aria-label="Tab actions"><Action disabled={busy} onClick={openLocalFiles}>{copy.workspace.openLocalFiles}</Action><Action disabled={busy || !(state.closedTabs?.length) || state.tabs.length >= MAX_TABS} onClick={() => setState(reopenDraft)}>{copy.workspace.reopenTab}</Action></div>
 ...
 {state.tabs.map(d => <section key={d.id} role="tabpanel" id={`sql-panel-${d.id}`} aria-labelledby={`sql-tab-${d.id}`} hidden={d.id !== active.id}>{d.id === active.id && <SqlEditor key={active.id} ref={editor} value={active.sql} from={active.from} to={active.to} schema={schema.data} dark={dark} error={editorError} onChange={sql => update(active.id, d => ({ ...d, sql }))} onSelection={(from, to) => update(active.id, d => ({ ...d, from, to }))} onRun={script => void execute(script)}/>}</section>)}
