...
const learnerExamples: Array<{ name: string; level: ExperienceLevel; description: string; sql: string; chart: Draft['chart']; parameters?: Record<string, string> }> = [
    ...
    { name: '06 · Query parameter.sql', level: 'expert', description: 'Use a typed parameter so the same query can answer different windows.', sql: 'WITH {days:UInt32} AS days\nSELECT\n    number AS day,\n    number * 4 AS events\nFROM numbers(days)\nORDER BY day', chart: { kind: 'line', x: 0, ys: [1], title: 'Parameterized events' }, parameters: { days: '7' } },
    { name: '07 · Explain a query.sql', level: 'expert', description: 'Inspect the plan before running a bounded read-only query.', sql: "EXPLAIN indexes = 1\nSELECT\n    toDate('2026-01-01') + number AS day\nFROM numbers(14)\nWHERE number % 2 = 0\nORDER BY day", chart: { kind: 'table', x: 0, ys: [], title: 'Query plan' } },
];
