...
export function Chart({ result, config, onFilter }: {
    ...
    useEffect(() => {
        ...
        void import('echarts').then(echarts => {
            ...
            const labels = result.rows.map(r => displayValue(r[config.x]));
            const series = config.ys.map(y => ({ name: result.columns[y]!.name, type: config.kind === 'area' ? 'line' : config.kind === 'stacked' ? 'bar' : config.kind,
                ...
                data: config.kind === 'pie' ? result.rows.map((r, i) => ({ name: labels[i], value: chartNumber(r[y]) })) : config.kind === 'scatter' ? result.rows.map(r => [chartNumber(r[config.x]), chartNumber(r[y])]) : result.rows.map(r => chartNumber(r[y])), connectNulls: false }));
            ...
            chart.on('click', params => { const index = (params as {
                dataIndex?: number;
            }).dataIndex; if (index !== undefined && result.rows[index])
                onFilter?.(x.name, result.rows[index]![config.x] === null ? null : displayValue(result.rows[index]![config.x])); });
    ...
    return <><div ref={element} className="chart-canvas" role="img" aria-label={config.title || 'Query result chart'}/>{error && <Callout danger>{error} The typed table remains available.</Callout>}<p className="muted">Chart coordinates use finite JavaScript numbers. Unsafe integer coordinates are omitted; the table and JSON retain exact values. Click a mark to draft a child filter.</p></>;
}
